...
class Game extends Phaser.Scene {
  ...
  preload() {
    ...
    let loadingItemText = this.make.text({
      x: width/2, 
      y: height/2+42,
      text: "",
      fontSize: 34,
  ...
  loadEventHandlers() {
    ...
    loadArena.play()
    this.krakenGrowls = []
    this.krakenGrowls.push(
      this.sound.add('kraken-growl-1'),
      this.sound.add('kraken-growl-2'),
      this.sound.add('kraken-growl-3'),
      this.sound.add('kraken-growl-4'),
    )
    this.krakenGrunt = this.sound.add('kraken-grunt')
    this.backgroundmusic = this.sound.add('background-music')
    if(this.getRandInt(1000) === 1) //this.getRandInt(200)
    // if(1 === 1)
    {
  ...
  addMap() {
    ...
    const backgroundSprite = this.add.sprite(this.map.widthInPixels / 2 - 40, this.map.heightInPixels / 2 + 50, 'bg-sprite').setOrigin(0.5, 0.5).setScrollFactor(0.9)
  ...
  fireKrakenCrystals(element, iteration, cycle) {
    ...
    switch(cycle) {
      case 0:
        if((900-xCoord) < this.gap || (870-xCoord) > this.gap + 120)
          this.krakenCrystals.fireCrystal(900-xCoord, -30, element)
        break
      case 1:
        if(xCoord < this.gap || xCoord > this.gap + 120)
          this.krakenCrystals.fireCrystal(xCoord, -30, element)
        break
      case 2:
        this.clearSafeZone()
