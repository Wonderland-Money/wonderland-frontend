...
import { StakingHelperContract, CupTokenContract, sCupTokenContract, StakingContract } from "../../abi";
...
export const changeApproval = createAsyncThunk("stake/changeApproval", async ({ token, provider, address, networkID }: IChangeApproval, { dispatch }) => {
    ...
    const timeContract = new ethers.Contract(addresses.CUP_ADDRESS, CupTokenContract, signer);
    console.log("Time contract: ");
    console.log(timeContract);
    const memoContract = new ethers.Contract(addresses.sCUP_ADDRESS, sCupTokenContract, signer);
